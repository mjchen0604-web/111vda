...
`ChatCore` 用于把 `new-api` 作为管理层，把内嵌 `chat` / `ChatMock` 作为真实模型执行层。

分工如下：
- `new-api` 负责用户、令牌、分组、额度、计费和管理后台
- `chat` 负责账号轮询、`service_tier`、模型映射和真实上游调用
...
5. `chat` 用自己的账号轮询和执行内核访问 ChatGPT backend
...
- `gpt-5.4-fast`、Codex、reasoning 是否可用，最终仍取决于 `chat` 这边的上游能力
