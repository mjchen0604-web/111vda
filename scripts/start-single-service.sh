...
if [[ -f "$CHATMOCK_CONFIG_SEED" && ! -f "$CHATGPT_LOCAL_HOME/config.toml" ]]; then
  cp "$CHATMOCK_CONFIG_SEED" "$CHATGPT_LOCAL_HOME/config.toml"
fi
...
export CHATGPT_LOCAL_REASONING_COMPAT="${CHATGPT_LOCAL_REASONING_COMPAT:-current}"
